...
module attributes { dlti.dl_spec = #dlti.dl_spec<#dlti.dl_entry<i64, dense<64> : vector<2xi64>>>} {
  ...
  reussir.func private @test_closure() -> !reussir.closure<(i32, i64) -> i32>

  // Test closure with different input/output types
  reussir.func private @test_closure_complex() -> !reussir.closure<() -> i64>

  // Test closure as function parameter
  reussir.func private @test_closure_param(%closure : !reussir.closure<(i32) -> i32>) -> i32

  // Test closure create op
  reussir.func private @test_closure_create() -> !reussir.rc<!reussir.closure<(i32) -> i32>> {
  ...
  reussir.func private @placeholder() {
    return
  }
  ...
  reussir.func private @test_closure_create_outlined() -> !reussir.rc<!reussir.closure<(i32) -> i32>> {
    %token = reussir.token.alloc : !reussir.token<align: 8, size: 32>
    %0 = reussir.closure.create -> !reussir.rc<!reussir.closure<(i32) -> i32>> {
      token(%token : !reussir.token<align: 8, size: 32>)
      vtable(@VTable)
