...
module {
    ...
    reussir.polyffi 
        texture("extern crate reussir_rt; use reussir_rt::collections::vec::Vec; [:NestedVec:] #[unsafe(no_mangle)] pub unsafe extern \"C\" fn __reussir_polyffi_Vec_[:typename:]_new() -> Vec<[:typename:]> {Vec::new()} #[unsafe(no_mangle)] pub unsafe extern \"C\" fn __reussir_polyffi_Vec_[:typename:]_drop(_ : Vec<[:typename:]>) {} #[unsafe(no_mangle)] pub unsafe extern \"C\" fn __reussir_polyffi_Vec_[:typename:]_push(vec : Vec<[:typename:]>, ele: [:typename:]) -> Vec<[:typename:]> {Vec::push(vec, ele)}")
